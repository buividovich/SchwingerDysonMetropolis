...
\end{eqnarray}
 ...
 Let us now simplify the ``effective mass'' term $\lr{1 + D_{x_1 z} \vev{G_{z x_1}}}$ for the standard kinetic operator $D_{x y} = \lr{2 D + m_0^2} \delta_{xy} + \sum\limits_{\pm \mu} \delta_{x\pm\hat{\mu}, y}$ on the isotropic lattice. A direct substitution yields
\begin{eqnarray}
\label{eff_mass_term}
 1 + \sum \limits_z D_{x_1 z} \vev{G_{z x_1}} = 
 1 + \lr{2 D + m_0^2} \vev{G_{x_1 x_1}} - \sum\limits_{\pm \mu} \vev{G_{x_1 x_1 \pm \hat{\mu}}}
 = \nonumber \\ = 
 1 + \frac{2 D + m_0^2}{\lambda} - \frac{2 D \vev{g^{\dag}_0 g_{0 + \hat{0}}} }{\lambda}
 = \lambda^{-1} \lr{\lambda + m_0^2 + 2 D \lr{1 - \vev{g^{\dag}_0 g_{0 + \hat{0}}}}}
\end{eqnarray}
...
\end{eqnarray}
where $K^2\lr{p} = m_0^2 + \sum\limits_{\mu} 4 \sin^2\lr{k_{\mu}/2}$ and $m^2 \equiv m_0^2 + \lambda + 2 D\lr{1 - \vev{g^{\dag}_x g_{x + \hat{0}}}}$. Introducing now the ``propagator''
\begin{eqnarray}
