...
\label{sm_partition_matrix_lagrange}
 \mathcal{Z} = \int dg_x \int d\phi_x \expa{ - \frac{N}{\lambda} \sum\limits_{x \neq y} D_{xy} \tr\lr{g^{\dag}_x g_y} - \frac{N}{\lambda} \sum\limits_x \tr\lr{\phi_x  g^{\dag}_x g_x - \phi_x} }
\end{eqnarray}
...
\label{sd_derivation2_lowest}
 \vev{G_{x y}} - \vev{\phi_x G_{x x}} \vev{G_{x y}}
 - \vev{G_{x x} \phi_x G_{x y} } + \lambda^{-1} \vev{\phi_x G_{x y}} = 0\\{}\nonumber\\
...
\end{eqnarray}
where we have also used the translational invariance to denote $\vev{\phi_{x_1} G_{x_1 x_1}} = \vev{\phi_t G_{tt}}$, where $t$ is an arbitrary point on the lattice. From the structure of these equations we see that the bare propagators acquire the effective mass term
\begin{eqnarray}
\label{eff_mass_lm}
 m^2 = \lambda \vev{\phi_t G_{tt}} = \lambda - \lambda G_{t z} D_{z t}
